...
if "Upload(UploadArgs)" not in s:
    anchor = "    /// Show detailed info for a single clip\n    Info(InfoArgs),"
    assert anchor in s, "Commands insertion anchor changed"
    s = s.replace(
        anchor,
...
'''
pattern = r'#\[derive\(clap::Args\)\]\npub struct CoverArgs \{.*?\n\}\n\n(?=#\[derive\(clap::Args\)\]\npub struct RemasterArgs)'
s, n = re.subn(pattern, cover, s, count=1, flags=re.S)
assert n == 1, "CoverArgs structural replacement failed"

if "pub struct UploadArgs" not in s:
    anchor = "#[derive(clap::Args)]\npub struct ListArgs {"
    assert anchor in s, "ListArgs insertion anchor changed"
    s = s.replace(
        anchor,
        '''#[derive(clap::Args)]
...
'''
pattern = r'        Commands::Cover\(args\) => \{.*?\n        \}\n\n(?=        Commands::Remaster\(args\) => \{)'
s, n = re.subn(pattern, cover_arm, s, count=1, flags=re.S)
assert n == 1, "cover match-arm replacement failed"

if "Commands::Upload(args)" not in s:
    anchor = "        Commands::Info(args) => {"
    assert anchor in s, "Info handler insertion anchor changed"
    arms = '''        Commands::Upload(args) => {
...
'''
    s = s.replace(anchor, arms + anchor, 1)
main.write_text(s)
